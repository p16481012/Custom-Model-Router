...
현재 버전은 **v0.6.11**이며, SillyTavern 1.18.0의 등록 가능한 Chat Completion 연결 24개, 공개 Registry API, 개발자 opt-in Routing API, 범용 DOM 모델 브리지, 호환성 진단과 비밀정보를 제외한 CMR 설정 백업·복구를 제공합니다. v0.6.10은 등록 모델이 12개를 넘을 때만 검색을 표시하고, 줄 단위 최대 200개 원자 등록, 삭제 직후 실행 취소, 백업 적용 전 추가·충돌·삭제 미리보기와 구체적인 설정 복구 사유를 추가했습니다. v0.6.11은 단일 모델 ID 입력과 여러 줄 등록을 하나의 textarea와 `+` 아이콘 버튼으로 합쳤습니다. 외부 연결 고급 메뉴는 실패·사용자 제외만 기본 목록에 두고, 정상 대상은 사용자가 명시적으로 펼치는 제외 대상 선택기에만 표시하며, Vectors 같은 비채팅 위험 대상은 관리 행이 아니라 진단 집계에만 남깁니다.
...
3. 제공업체를 선택하고 모델 ID 입력란에 업체가 공개한 정확한 ID를 입력합니다. 하나만 입력해도 되고, 여러 ID라면 한 줄에 하나씩 최대 200개를 같은 입력란에 넣은 뒤 `+` 아이콘을 누릅니다.
...
Playwright Chromium UI 회귀 검사 10개는 제품 `settings.html`을 SillyTavern 1.18.0 고정 commit의 `style.css`·`popup.css`와 함께 렌더링합니다. 320×568, 360×640, 420×800, 720×900에서 가로 넘침, 버튼 정렬, 공백 단위 줄바꿈, 단일 닫기, 단일·여러 줄 공용 textarea와 아이콘 전용 등록 버튼, 예외 중심 고급 외부 연결 관리 UI를 검사합니다. 모델 목록 경계, 12개 초과 검색, 여러 줄 원자 등록·삭제 실행 취소, 백업 미리보기, 조건부 문제 카드에서 고급 관리로 이동하는 흐름과 문제 대상 제외·복구 뒤 포커스 유지도 검증합니다. 모델·외부 대상·진단 목록은 마우스 휠과 키보드로, Popup 본문은 마우스 휠로 실제 스크롤합니다. GitHub Actions는 성공과 실패 모두 PNG와 HTML report를 `ui-regression-evidence-*` artifact로 14일 보관합니다.
...
| `v0.6.11` | 단일·여러 줄 모델 등록을 공용 textarea와 아이콘 버튼 하나로 통합 | ✅ 현재 릴리스·🧪 사용자 검증 대기 |
