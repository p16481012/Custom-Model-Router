...
현재 버전은 **v0.6.17**입니다. SillyTavern 1.18.0의 Chat Completion 연결 24개에 사용자 모델을 등록하고, 안전한 외부 모델 칸과 공개 API에 제공합니다. 이번 버전은 백업 검증·선택 복원·스트리밍 취소·hook 격리·이벤트 순서를 보완하고, 모델 활성화·비활성화, 선택적 모델 병합, 설정 되돌리기, 기본 모델 중복 정리와 연동별 재시도를 추가합니다.
...
- **모델 정리 및 복구**에서 기본 목록에 편입된 등록을 선택적으로 정리하거나 최근 백업 적용·일괄 정리 전 상태를 되돌립니다. 둘 다 미리보기 뒤 적용하며, 기본 모델 자체는 삭제하지 않습니다.
...
| Chromium UI 회귀 검사 | ✅ 실제 `settings.html`, SillyTavern 1.18.0 CSS와 native 재사용·공용 provider hook fixture 및 제품 진입점 검사 23개 통과 |
...
| `v0.6.17` | 결함 6개 보완, 모델 토글·선택 병합·설정 되돌리기·중복 정리·연동별 재시도 | ✅ 현재 릴리스·🧪 실제 계정 검증 대기 |
