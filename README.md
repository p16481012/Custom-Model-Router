...
SillyTavern 코어 파일을 수정하지 않고 기존 Chat Completion 연결에 사용자 모델 ID를 등록하는 UI 확장입니다. 관리 팝업의 기본 목록은 등록한 모든 모델을 제공업체별로 보여주며, 실제 모델은 SillyTavern의 기존 모델 선택기 또는 입력란에서 선택합니다. 범용 외부 확장 브리지는 안전하게 감지한 모델 컨트롤에 native option과 중복되지 않는 등록 모델을 target별 최대 512개까지 제공업체별로 자동 추가하며 외부 요청 본문을 직접 바꾸지 않습니다.

현재 버전은 **v0.6.10**이며, SillyTavern 1.18.0의 등록 가능한 Chat Completion 연결 24개, 공개 Registry API, 개발자 opt-in Routing API, 범용 DOM 모델 브리지, 호환성 진단과 비밀정보를 제외한 CMR 설정 백업·복구를 제공합니다. v0.6.10은 등록 모델이 12개를 넘을 때만 검색을 표시하고, 줄 단위 최대 200개 일괄 등록, 삭제 직후 실행 취소, 백업 적용 전 추가·충돌·삭제 미리보기와 구체적인 설정 복구 사유를 제공합니다. 외부 연결 고급 메뉴는 실패·사용자 제외만 기본 목록에 두고, 정상 대상은 사용자가 명시적으로 펼치는 제외 대상 선택기에만 표시하며, Vectors 같은 비채팅 위험 대상은 관리 행이 아니라 진단 집계에만 남깁니다.
...
| Chromium UI 회귀 검사 | ✅ 실제 `settings.html`과 SillyTavern 1.18.0 CSS 결합 검사 10개 통과 |
...
- 등록 모델이 12개를 넘을 때만 제공업체 이름·ID·모델 ID 검색을 표시합니다. 줄마다 모델 ID 하나씩 최대 200개를 일괄 등록할 수 있고, 한 줄이라도 형식이 잘못되면 전체 묶음을 적용하지 않습니다.
...
3. 제공업체를 선택하고 업체가 공개한 정확한 모델 ID를 등록합니다. 여러 ID라면 **여러 모델 한꺼번에 등록**을 펼쳐 한 줄에 하나씩 최대 200개를 입력합니다.
...
|---|---|---|
| Caption | provider/model 표준 DOM 탐지, option `data-type` provider metadata 보존, 선택 이벤트와 재렌더 복원 | `/caption-image` 요청 payload의 `model` 및 계정·모델 멀티모달 호환은 사용자 확인 대기 |
...
현재 자동 검사 207개는 24개 provider 회계, 전체 등록 모델 UI와 SillyTavern native 선택 경계, 12개 초과 조건부 검색, 최대 200줄 원자적 일괄 등록, 삭제 실행 취소, 공개 API의 `model_in_use` 보호, schema 이관, 동적 옵션 복원, 팝업 수명주기, 개발자용 라우팅 격리, 외부 select/input/datalist 선택지 주입, 동일 구조 target 구분, provider별 그룹 주입·재렌더·정리, 예외 중심 외부 관리 UI, target별 native 중복 제외 후보 512개·전체 예상/실제 CMR option 합계 2,048개 경고, risk-only·사용자 제외-only 화면의 Registry 재고 계측, schema v2 사용자 제외·복구, 비대상 제외, 진단 schema v2 합계·계측 대기·최신 복사, 구체적 복구 사유, 백업 추가·충돌·삭제 미리보기와 버전·문서 일치를 검증합니다. 실제 계정과 브라우저 조작이 필요한 결과는 [통합 사용자 체크리스트](./USER_CHECKLIST.md)에서 별도로 확인합니다.
...
Playwright Chromium UI 회귀 검사 10개는 제품 `settings.html`을 SillyTavern 1.18.0 고정 commit의 `style.css`·`popup.css`와 함께 렌더링합니다. 320×568, 360×640, 420×800, 720×900에서 가로 넘침, 버튼 정렬, 공백 단위 줄바꿈, 단일 닫기, 아이콘 전용 추가 버튼과 예외 중심 고급 외부 연결 관리 UI를 검사합니다. 모델 목록 경계, 12개 초과 검색, 일괄 등록·삭제 실행 취소, 백업 미리보기, 조건부 문제 카드에서 고급 관리로 이동하는 흐름과 문제 대상 제외·복구 뒤 포커스 유지도 검증합니다. 모델·외부 대상·진단 목록은 마우스 휠과 키보드로, Popup 본문은 마우스 휠로 실제 스크롤합니다. GitHub Actions는 성공과 실패 모두 PNG와 HTML report를 `ui-regression-evidence-*` artifact로 14일 보관합니다.
...
| `v0.6.10` | 대량 모델 관리, 삭제 실행 취소, 백업 미리보기·복구 상세, 예외 중심 외부 관리와 DOM option 경고 | ✅ 현재 릴리스·🧪 사용자 검증 대기 |
