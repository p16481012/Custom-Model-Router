...
SillyTavern 코어 파일을 수정하지 않고 기존 Chat Completion 연결에 사용자 모델 ID를 등록하는 UI 확장입니다. 관리 팝업에는 직접 등록한 모델만 표시합니다. 범용 외부 확장 브리지는 현재 로드된 기본 모델과 등록 모델을 합쳐 안전한 모델 칸에 target별 최대 512개까지 제공합니다. 공개 provider hook에 opt-in한 외부 확장은 선택된 Connection Manager 프로필을 CMR 요청 handler로 상속할 수 있습니다. hook이 없는 명확한 Custom/OpenAI-compatible 또는 SillyTavern 현재 연결 provider option에는 그 의미에 맞는 모델만 투영하며, 실제 handler 사용 여부는 별도로 확인해야 합니다.

현재 버전은 **v0.6.22**입니다. SillyTavern 1.18.0의 Chat Completion 연결 24개를 지원합니다. 외부 확장이 현재 모델만 가져오더라도, SillyTavern에 로드된 나머지 기본 모델을 CMR에 중복 등록할 필요 없이 지원되는 외부 모델 칸에서 선택할 수 있습니다. 목록에 없는 모델만 직접 등록하면 됩니다.
...
### v0.6.22 외부 제공업체별 모델 필터 수정

- 외부 확장에서 OpenAI·Claude·Google·Vertex 등 제공업체를 선택하면 그 업체의 기본·등록 모델만 표시합니다. 제공업체 전환 시 이전 업체의 CMR 옵션과 입력 제안을 제거합니다.
- 비어 있거나 알 수 없는 선택, 충돌하는 표식, 모호한 제공업체 연결에는 다른 업체 모델을 대신 넣지 않습니다. 명시적 제공업체 속성도 따르며, 제공업체 제한이 없는 독립 모델 칸만 기존 전체 목록을 유지합니다.
- 기존 native 옵션·제공업체 설정·메인 연결과 Custom·현재 SillyTavern 연결의 전용 처리, CMR 관리 팝업의 전체 등록 목록은 유지합니다.
- 개수만 보던 기존 검사를 모델 ID·소속 제공업체 대조로 강화했습니다. 실제 외부 handler나 원격 요청 지원을 추가한 것은 아닙니다.

### v0.6.21 입력 오류 수정 지원

- 입력·붙여넣기 후 신규·중복·형식 오류 개수를 미리 표시합니다. 빈 입력에서는 숨기며, 중복은 등록을 막지 않고 건너뜁니다.
- 오류·중복 행을 누르거나 키보드로 실행하면 해당 행을 선택하고 스크롤합니다. 긴 ID의 화면 줄바꿈도 반영하며 입력 내용과 기본 실행 취소 기록을 바꾸지 않습니다.
- 한글 조합 중에는 검사를 보류하고, 제공업체·등록 목록이 바뀌면 다시 확인합니다. 등록 버튼을 누를 때도 재검사하며 형식 오류가 있으면 일부만 등록하지 않습니다.
- 로컬 형식·중복 검사이며 실제 모델 존재·계정 권한·요청 성공을 확인하지 않습니다. 다른 편의 기능이나 API·백업 계약은 변경하지 않습니다.

### v0.6.20 기본 목록 자동 제공

- 현재 로드된 core 모델 목록과 CMR 등록 목록을 `(제공업체, 모델 ID)`로 합칩니다. Custom은 실제 로드된 select·datalist를 참조하며, 임의 입력값이나 아직 불러오지 않은 원격 모델을 추측하지 않습니다.
- 기본 모델은 등록 목록·백업·Registry API에 복사하지 않습니다. 외부 DOM 브리지와 opt-in한 공용 provider hook에만 함께 제공합니다. 추가 네트워크 요청은 없습니다.
- 기존 수동 등록은 삭제하지 않습니다. 비활성화한 등록은 같은 기본 모델의 자동 제공도 막으며, 등록을 삭제하면 현재 기본 목록에 있는 모델은 다시 자동 제공됩니다. core의 원래 옵션은 지우지 않습니다.
- 현재 선택과 native 옵션을 보존합니다. 목록에서 실제로 사라진 선택은 기존 fallback 정책을 따릅니다. 512개 표시 한도에서는 현재 선택과 직접 등록한 모델을 우선하며, 초과 경고를 유지합니다.
...
- **기본 모델 중복 정리**는 선택적인 등록 삭제 기능입니다. v0.6.20부터 정리한 기본 모델도 현재 로드된 목록에서 자동 제공됩니다. 항상 수동 등록을 유지하려는 항목은 미리보기에서 제외하세요.

### v0.6.18 중복 정리 바로가기

- **전체 등록 모델** 제목 옆 빗자루 아이콘으로 **기본 모델 중복 정리**를 바로 엽니다. 버튼 이름은 툴팁과 스크린 리더에서, 정리 범위는 목록의 정보 아이콘에서 확인합니다.
- 미리보기는 목록 바로 아래에 표시됩니다. 메뉴를 따로 열 필요 없으며, 확인·적용 전에는 등록을 제거하지 않습니다. 취소·적용 뒤에는 해당 버튼으로 초점이 돌아옵니다.
- CMR 텍스트 버튼에 SillyTavern의 `width: min-content`가 남아 문구가 여러 줄로 쌓이던 문제를 수정합니다. 기존 정사각형 아이콘 크기는 유지합니다.

### v0.6.17에서 추가된 관리 기능

- 모델 행의 토글 아이콘으로 활성화·비활성화합니다. 현재 사용 중인 select형 custom-only 모델은 먼저 다른 모델로 전환해야 합니다.
- 백업 미리보기에서 **전체 교체** 또는 **모델만 병합**을 고릅니다. 병합은 체크한 모델만 반영하고 기존 모델·경로·외부 설정을 유지합니다. 활성 상태 충돌은 기본적으로 현재 값 유지이며, 체크하면 백업의 상태를 사용합니다.
- 목록 제목 옆 **기본 모델 중복 정리**로 기본 목록에 편입된 등록을 선택적으로 정리합니다. **호환성 진단 및 CMR 설정 백업 → 설정 복구**에서는 최근 백업 적용·일괄 정리 전 상태를 되돌립니다. 둘 다 미리보기 뒤 적용하며, 기본 모델 자체는 삭제하지 않습니다.
- 되돌리기는 현재 실행의 직전 상태 한 건만 메모리에 보관합니다. 팝업을 닫아도 유지되지만 페이지 새로고침·확장 종료 시 만료됩니다. 손상된 원본 저장값이나 인증 설정을 저장·복원하는 기능은 아닙니다.
- 고급 외부 관리의 **공용 연동**에는 등록된 hook consumer가 있을 때만 상태와 개별 재시도가 나타납니다. 정상 consumer는 다른 consumer의 대기와 격리되며, hook 응답은 기본 10초 후 시간 초과로 처리합니다. 이 제한은 실제 모델 생성 시간 제한이 아닙니다.

v0.6.17은 잘못된 JSON 루트 거부, 중복 모델 ID의 공급자 복원, 공유 datalist 격리, Routing 스트림 취소, hook 대기 격리와 Registry 이벤트 재진입도 수정합니다. 등록 총량과 백업 총량은 모두 5,000개입니다.

새 브라우저 검사는 실제 `index.js`·마크업·스타일·기능 모듈을 사용합니다. SillyTavern context와 Popup 호스트는 테스트 대역이며, 실제 설치된 외부 확장 및 제공업체 네트워크 검증을 대신하지 않습니다. 현재 자동 UI 검증 엔진은 Chromium입니다.
...
|---|---|
| 현재 릴리스 | `v0.6.22` |
| v0.6.22 외부 제공업체 필터 | ✅ 선택 업체의 기본·등록 모델만 표시, 업체 전환·미지원·모호한 연결 정리 |
| v0.6.21 입력 오류 수정 지원 | ✅ 입력·붙여넣기 사전 검사, 오류·중복 행 선택과 스크롤, IME·실행 취소 보존 |
| v0.6.20 기본 목록 자동 제공 | ✅ 등록 없는 기본 모델 제공, provider·ID 중복 제거, 저장·백업 분리와 동적 목록 갱신 |
...
| Chromium UI 회귀 검사 | ✅ 실제 `settings.html`, SillyTavern 1.18.0 CSS와 native 재사용·공용 provider hook fixture 및 제품 진입점 검사 31개 통과 |
...
- 다른 확장의 표준 Chat Completion 모델 컨트롤을 자동 탐지해 선택된 제공업체의 기본·등록 모델만 target별 최대 512개까지 추가합니다. 제공업체 제한이 없는 독립 모델 칸은 기존 제공업체별 전체 목록을 유지하며 native 중복은 제외합니다.
...
- `CustomModelRouter.integrations`에 opt-in한 외부 확장에는 선택된 Connection Manager 프로필 기반의 provider 요청 handler와 해당 provider의 가용 기본·등록 모델만 제공합니다.
...
Registry API 계약은 `1.2.0`, Provider Integration API 계약은 `1.1.0`, Routing API 계약은 `1.0.0`입니다. Registry `1.2.0`은 `integrations`를 추가한 하위 호환 minor 갱신이며 기존 Routing API의 호출 계약은 바뀌지 않았습니다. 내부 `src` 파일을 직접 import하지 말고 확장 초기화 뒤 전역 객체를 사용합니다.
...
2. **OpenAI-compatible 특화**: 현재 선택된 프로필이 SillyTavern `Custom` source일 때만 `openai-compatible` handler와 `custom`의 가용 기본·등록 모델을 준비합니다.
...
- 정확한 Custom/OpenAI-compatible 선택: `custom`의 가용 기본·등록 모델만 투영
- 정확한 SillyTavern/current-connection 선택: 현재 활성 SillyTavern provider의 가용 기본·등록 모델만 투영
...
| `v0.6.19` | 기본 모델 등록 허용, 외부의 부족한 모델 목록 보완, 대상별 옵션·현재 선택 보존 | ✅ 완료 |
| `v0.6.20` | 기본 목록 자동 제공, 등록·백업 분리, 동적 갱신과 직접 등록 우선 | ✅ 완료·🧪 실제 외부 확장 검증 대기 |
| `v0.6.21` | 입력 오류·중복 사전 검사와 문제 행으로 이동 | ✅ 완료·🧪 실제 설치 화면 검증 대기 |
| `v0.6.22` | 외부의 선택된 제공업체만 모델 표시, 전환·미지원·모호한 연결 정리 | ✅ 현재 릴리스·🧪 실제 외부 확장 검증 대기 |
