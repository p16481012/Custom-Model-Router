...
- 한 관리 팝업에서 24개 제공업체의 사용자 모델을 등록·삭제합니다.
...
- 제공업체를 판별할 수 없거나 채팅 모델이 아닌 대상은 변경하지 않으며, 대상별 수동 연결·자동·확인·사용 안 함 UI를 제공하지 않습니다.
...
- Registry, 개발자용 용도별 경로와 외부 컨트롤의 provider별 마지막 선택만 portable schema v2 JSON으로 내보내고, 가져오기 전 형식·스키마·허용 필드를 검사합니다.
...
4. 관리 팝업을 닫고 SillyTavern의 기존 모델 선택기 또는 입력란에서 등록 모델을 선택합니다.
5. 일반 요청과 스트리밍 요청을 각각 확인합니다.

등록은 해당 source가 비활성 상태여도 가능합니다. 관리 팝업의 등록 목록에는 선택 버튼이나 현재 모델 상태가 없으며, 실제 선택 상태는 SillyTavern의 기존 컨트롤을 기준으로 합니다.
...
범용 브리지는 페이지에 나타난 표준 Chat Completion 모델 `select`, 텍스트 `input`, `datalist`를 자동 탐지합니다. 제공업체를 충분히 확실하게 판별한 대상에는 해당 Registry 모델만 추가하고, 판별할 수 없는 대상은 그대로 둡니다. Caption처럼 provider 선택기와 model 선택기, option의 `data-type`을 사용하는 확장은 그 값을 보존한 채 연결합니다.
...
표준 모델 컨트롤이 없거나 제공업체를 확실하게 판별할 수 없는 확장은 CMR의 Registry 또는 Routing API를 사용하는 전용 opt-in 연동이 필요합니다. 자동 탐지와 비대상 판별은 DOM의 이름·레이블·속성·상위 확장 표식을 사용하는 best-effort 호환 기능이며 모든 외부 확장의 요청 규격을 보증하지 않습니다. 알 수 없는 대상에는 CMR 모델을 임의로 추가하지 않습니다.
...
- 외부 모델 컨트롤의 자동 provider 판별 성공·판별 불가·안전상 건너뜀 개수
