...
현재 단계: **v0.6.2에서 v0.6.1 관리 팝업 UI 변경을 롤백하고, 사용자와 기능 역할·정보 구조를 재논의 중**
...
- [ ] 등록 목록에서 선택 기능이 필요한지 사용자와 재결정
- [ ] 외부 확장에는 provider별 전체 등록 모델을 자동 표시할지 사용자와 재결정
- [ ] 기능별 라우팅을 일반 UI에서 유지·고급 기능으로 이동·숨김 중 하나로 재결정

합의 전까지 새로운 관리 UI 구현은 진행하지 않습니다.
...
실제 검증에서 발견되는 v0.6 범위의 후속 결함은 `v0.6.3`, `v0.6.4`, ... 패치 버전으로 수정합니다.
...
| v0.6.2 | ✅ 현재 릴리스 | v0.6.0 UI 복원, 다음 UI 방향 재논의 |
