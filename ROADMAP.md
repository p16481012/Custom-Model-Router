...
현재 단계: **v0.6.6에서 외부 연결을 단일화하고 진단 오탐·중복 닫기·추가 버튼을 수정해 사용자 검증 대기 중**
...
| 샌드박스와 자동 통합 검사 | ✅ 완료 | 동작 샌드박스는 기본 24개·native 선택·외부 모델 컨트롤·개발자 route API를 확인. 단일 직접 연결은 자동 통합 검사로 확인했으며, 좁은 화면 배치는 수동 DOM/CSS 샌드박스로 분리 |
...
실제 검증에서 발견되는 v0.6 범위의 후속 결함은 `v0.6.7`, `v0.6.8`, ... 패치 버전으로 수정합니다.
...
| v0.6.6 | ✅ 현재 릴리스·🧪 사용자 검증 대기 | 외부 연결 단일화, 중복 자원 진단 오탐·중복 닫기·추가 버튼 수정 |
