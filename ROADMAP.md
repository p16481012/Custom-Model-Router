...
현재 단계: **v0.6.22 외부 제공업체별 모델 필터 수정 및 자동 검사 완료, 실제 외부 확장 검증 대기**
...
| 자동 검사 | ✅ 완료 | 단위·통합·수명주기·보안 경계·버전 일치 검사 276개 통과 |
| DOM·공개 API 샌드박스 | ✅ 완료 | 기본 24개·native 선택·외부 모델 컨트롤, provider/source 선택기 보존·개발자 route API와 정리 수명주기 확인 |
| Chromium UI 회귀 검사 | ✅ 완료 | 실제 `settings.html`, SillyTavern 1.18.0 고정 CSS, 기본 모델 등록·중복 방지·현재 선택 보존과 브라우저 native 재사용·공용 provider hook fixture를 확인하는 Playwright Chromium UI 회귀 검사 31개 통과 |
...
## v0.6.17 — 안정성 보완과 선택적 모델 관리

상태: **✅ 구현·자동 검사 완료, 🧪 실제 계정 검증 대기**

- [x] falsy JSON 루트를 정상 백업으로 승인하지 않음
- [x] 동일 value의 다른 공급자 select 복원과 공유 datalist의 입력별 격리
- [x] Routing 스트림 종료까지 취소·수명 추적 유지
- [x] consumer별 직렬 큐, hook 응답 시간 제한, 늦은 영수증 정리와 개별 재시도
- [x] Registry 이벤트 batch revision 고정과 재진입 mutation 순차 알림
- [x] 모델 활성 토글, 현재 custom-only 모델 비활성화 보호
- [x] 모델만 선택 병합, 충돌 시 현재 상태 기본 유지, 기존 경로·외부 설정 보존
- [x] 백업 적용·일괄 정리 직전 상태 한 건의 세션 내 되돌리기와 미리보기
- [x] 기본 목록 중복 등록 정리 미리보기와 적용 직전 native 상태 재확인
- [x] 등록/백업 5,000개 한도 통일, 미사용 상수·스타일 제거
- [x] Node 자동 검사 255개와 Playwright Chromium UI 회귀 검사 23개로 기존 계약과 신규 제품 진입점 흐름을 검증

되돌리기에는 portable 허용 필드만 보관하며 손상된 원본 저장값이나 인증 설정은 복원하지 않습니다. 기존 v0.1 호환 래퍼와 백업 이관은 유지합니다. 큰 파일의 전면 분리, 일반 조회 캐시, Firefox/WebKit 자동 검증은 이번 패치에 포함하지 않습니다.

## v0.6.18 — 중복 정리 바로가기와 버튼 폭 수정

상태: **✅ 구현·자동 검사 완료, 🧪 실제 설치 화면 검증 대기**

- [x] 기본 모델 중복 정리를 전체 등록 모델 제목 옆의 이름 있는 빗자루 아이콘으로 이동
- [x] 공용 변경 미리보기를 목록 아래로 이동해 진단·백업 메뉴가 접혀 있어도 확인·적용 가능
- [x] 중복 정리·설정 복구 취소 문구와 시작 버튼으로의 초점 복원, 완료 후 숨겨진 되돌리기 버튼의 안전한 복귀 처리
- [x] CMR 텍스트 버튼의 `width: min-content` 상속 해제, 정사각형 아이콘과 설정 되돌리기의 충분한 폭 유지
- [x] Node 자동 검사 256개와 Playwright Chromium UI 회귀 검사 24개로 기존 계약, 접힌 메뉴에서의 정리·취소·재확인과 320·360·420·720px의 밝은 테마 아이콘 배치를 검증

기본 모델 자체·현재 native 선택·연결 설정은 바꾸지 않으며, 중복 등록 제거는 기존처럼 미리보기 확인 후에만 적용합니다. API 계약과 백업 형식은 변경하지 않습니다.

## v0.6.19 — 기본 모델 등록 허용

상태: **✅ 구현·자동 검사 완료, 🧪 실제 외부 확장 검증 대기**

- [x] 기본 목록에 있다는 이유로 UI 등록을 거부하던 native 중복 판정 제거
- [x] CMR의 동일 제공업체·ID와 입력 내 반복만 중복 처리, 잘못된 행의 원자적 거부 유지
- [x] 대상별 기존 옵션 중복 방지와 현재 선택·native DOM·change 이벤트 보존
- [x] 기본 모델 등록 안내, 외부에서 쓰는 등록을 정리하지 않도록 미리보기 주의사항 추가
- [x] Node 자동 검사 257개와 Playwright Chromium UI 회귀 검사 25개로 단일·여러 줄 기본 모델 등록, 현재 모델만 가진 외부 선택기 보완, 기존 옵션 재사용과 재초기화 후 등록 보존을 검증

기본 카탈로그 전체를 자동 복사하지는 않으며, 사용자가 입력한 모델만 등록합니다. API·백업 형식과 외부 요청 handler 지원 범위는 바꾸지 않습니다.
...
- [x] 외부 DOM 브리지·공용 provider hook에 병합 목록 제공, hook 요청 직전 가용성 재검사
- [x] Registry API·등록 목록·백업은 직접 등록만 유지, 기존 등록 자동 삭제나 원격 모델 조회 없음
- [x] 목록 교체·ID·disabled·hidden 변경 동기화, core 옵션·선택 보존, 사라진 외부 선택은 기존 fallback 정책 유지
- [x] target별 512개 한도에서 현재 선택·직접 등록 우선, 기존 용량·성능 경고 유지
- [x] Node 자동 검사 266개와 Playwright Chromium UI 회귀 검사 27개로 기본 목록 합치기, 저장 분리, Custom 목록·hook, 동적 갱신과 수동 등록 override를 검증
- [ ] 실제 설치된 외부 확장 handler와 사용자 계정 요청 확인 — 자동 검사가 대신하지 않음

## v0.6.21 — 입력 오류 수정 지원

사용자 편의성 제안 중 입력 오류 수정 지원만 적용합니다.

- [x] 기존 등록 검증 규칙을 재사용해 입력·붙여넣기 후 신규·중복·형식 오류 개수와 행별 원인 표시
- [x] 오류·중복 행 버튼으로 입력란의 해당 행 선택·스크롤, 긴 ID의 화면 줄바꿈 반영
- [x] 빈 입력에서는 숨김, 한글 조합 중 검사 보류, 제공업체·등록 변경 반영과 오래된 행 링크 방어
- [x] 미리보기는 저장·네트워크 요청 없음, 입력 내용·기본 실행 취소 보존, 등록 시 재검사와 원자적 등록 유지
- [x] Node 자동 검사 273개와 Playwright Chromium UI 회귀 검사 30개로 실제 붙여넣기, 키보드 행 이동, 수정·실행 취소, IME 이벤트, 320·360·420·720px 긴 입력 스크롤과 팝업 수명주기를 검증
- [ ] 실제 설치 환경의 테마·모바일 키보드와 스크린 리더 확인

형식·중복 검사이지 실제 모델 호출 검증이 아닙니다. 다른 편의 기능, 공개 API와 백업 형식은 변경하지 않습니다.

## v0.6.22 — 외부 제공업체별 모델 필터 수정

일반 제공업체 선택을 무시하고 전체 모델을 넣던 오류를 수정합니다. Custom·현재 ST 연결만 필터링하던 이전 동작과 전체 목록을 정답으로 삼던 검사도 함께 바로잡습니다.

- [x] 선택된 일반 제공업체와 명시적 provider 속성의 범위로 기본·등록 모델 및 input/datalist 제안 제한
- [x] 업체 전환 시 이전 CMR 옵션 정리, native 옵션·제공업체 설정·메인 연결 보존
- [x] 빈 선택·미지원·충돌·비활성·모호한 연결·사라진 명시 참조에서 전체 목록 fallback 차단
- [x] 제공업체 control 재생성 및 metadata 변경 추적, Custom·현재 연결의 엄격한 기존 계약 유지
- [x] Node 자동 검사 276개와 Playwright Chromium UI 회귀 검사 31개로 24개 제공업체·외부 별칭, 모델 ID·제공업체 대조, 기본·수동 모델과 입력 제안·전환·정리·control 교체를 검증
- [ ] 실제 설치한 외부 확장에서 선택 목록과 요청 model 확인

제공업체 제한 없는 독립 모델 칸과 CMR 관리 팝업의 전체 등록 목록은 변경하지 않습니다. 공개 API·백업 형식이나 외부 요청 handler 지원 범위도 그대로입니다.
...
| v0.6.16 | ✅ 구현 완료·🧪 사용자 검증 대기 | SillyTavern 공식 Popup 닫기의 CMR 패널 내부 배치와 좁은 화면 회귀 방지 |
...
| v0.6.22 | ✅ 현재 릴리스·🧪 사용자 검증 대기 | 외부의 선택 업체별 모델 필터, 전환·미지원·모호한 연결 정리 |
