...
현재 단계: **24개 Chat Completion 연결 구현·자동 검사 완료, 실제 제공업체 계정 검증 대기**
...
| v0.2 안정화 | 🧪 대기 | 실제 실패가 확인되면 `v0.2.1+` 패치로 수정 |
...
실제 검증에서 확인되는 버그와 세부 호환성 수정은 `v0.2.1`, `v0.2.2`, ... 패치 버전으로 반영합니다.
...
- 별칭 변경·삭제 시 참조 무결성
- 주 모델과 보조 모델의 빠른 전환 UI

완료 조건: 별칭 생성·수정·삭제·해석과 저장·복원 자동 검사를 모두 통과합니다.
...
완료 조건: 공개 API 계약 문서, 변경 이벤트, 연동 예제와 독립 계약 테스트를 제공합니다.
...
| v0.2.0 | 🧪 현재 | 24개 Chat Completion 연결 구현·자동 검사 완료, 실제 계정 검증 대기 |
