...
현재 단계: **v0.6.11의 단일·여러 줄 공용 모델 ID 입력 UI를 구현하고 자동 검사를 마친 뒤 실제 사용자 환경 검증 대기 중**
...
| 관리 팝업과 기본 컨트롤 | ✅ 완료 | 24개 제공업체 모델 등록·삭제, 단일·여러 줄 공용 textarea와 아이콘 버튼 하나, 전체 provider 목록, 12개 초과 조건부 검색, 최대 200줄 원자 등록, 삭제 실행 취소와 6개 초과 숨은 스크롤 |
...
실제 검증에서 발견되는 v0.6 범위의 후속 결함은 `v0.6.12`, `v0.6.13`, ... 패치 버전으로 수정합니다.
...
| v0.6.11 | ✅ 현재 릴리스·🧪 사용자 검증 대기 | 단일·여러 줄 모델 등록을 공용 textarea와 아이콘 버튼 하나로 통합 |
