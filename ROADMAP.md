...
현재 단계: **v0.6.1 관리 팝업 UI·UX 정리와 자동 검사 완료, 사용자 환경에서 외부 확장 실제 요청 확인 대기**
...
| 관리 팝업 UI·UX | ✅ 완료 | 카드형 정보 구조, 충분한 여백, 반응형 버튼 정렬, 일반 사용자용 상태 설명과 단일 연결 방식 메뉴 |
...
| 자동 검사 | ✅ 완료 | 단위·통합·UI 구조·수명주기·보안 경계·버전 일치 검사 131개 통과 |
...
- 비대상 판별도 DOM 표식 기반 best-effort입니다. 용도를 드러내지 않는 일반적인 모델 컨트롤이 잘못 감지되면 사용자가 대상별 **연결 안 함**을 지정합니다.
...
## v0.6.1 — 관리 팝업 UI·UX 정리

상태: **✅ 구현·자동 검사 완료, 사용자 화면 검증 대기**

### 반영 결과

- [x] 모델 등록 영역과 추가 기능을 독립된 카드와 일관된 내부 여백으로 구분
- [x] 다른 확장 연결의 `자동·수동·확인 필요·제외` 내부 용어를 일반 사용자용 설명으로 교체
- [x] 여러 개의 `연결·자동·제외` 버튼을 `연결 방식` 선택 메뉴 하나로 통합
- [x] 기술 선택자(`#caption_multimodal_model` 등)를 기본 제목에서 분리해 상세 영역의 기술 정보로 이동
- [x] 일반적인 Model 입력란도 확장·컨트롤 표식을 바탕으로 서로 구분되는 이름으로 표시
- [x] 기능별 모델 지정 버튼 3개를 같은 행의 동일 너비로 정렬
- [x] 진단과 백업을 서로 다른 하위 카드로 나누고, 버튼의 한 글자 세로 줄바꿈을 방지
- [x] 백업 불러오기를 키보드로 실행할 수 있는 실제 버튼으로 교체
- [x] 모든 버튼·select·input·summary에 일관된 `focus-visible` 표시 추가
- [x] 설정 필요 항목의 초기값을 `제공업체를 선택하세요`로 표시하고 자동 감지 재시도를 별도 선택지로 분리
- [x] 좁은 폭에서도 `현재 사용` 텍스트를 유지하고 초협폭에서는 버튼 겹침을 막는 CSS fallback 추가
- [x] 320~420px 폭의 버튼·카드 반응형 CSS 계약 검사

### 사용자 확인이 필요한 항목

- [ ] 실제 SillyTavern 테마에서 밝은·어두운 색상 모두 카드 경계와 설명이 읽기 쉬움
- [ ] 320~420px 폭에서 버튼 문구가 세로로 찌그러지거나 겹치지 않고, 일반 폭의 기능별 버튼은 한 행으로 표시됨
- [ ] `다른 확장에서도 모델 사용`의 네 상태 설명과 `연결 방식` 선택만으로 설정 방법을 이해할 수 있음
...
실제 검증에서 발견되는 v0.6 범위의 후속 결함은 `v0.6.2`, `v0.6.3`, ... 패치 버전으로 수정합니다.
...
| v0.6.0 | ✅ 구현 완료 | 범용 DOM 모델 브리지, 수동 mapping, 외부 연결 portable schema v2 |
| v0.6.1 | ✅ 현재 구현 완료·🧪 사용자 검증 대기 | 관리 팝업 카드 구조, 반응형 버튼 정렬, 쉬운 연결 상태와 단일 연결 방식 UI |
