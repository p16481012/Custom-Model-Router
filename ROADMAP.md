...
현재 단계: **v0.6.20 기본 목록 자동 제공 및 자동 검사 완료, 실제 외부 확장 검증 대기**
...
| 자동 검사 | ✅ 완료 | 단위·통합·수명주기·보안 경계·버전 일치 검사 266개 통과 |
| DOM·공개 API 샌드박스 | ✅ 완료 | 기본 24개·native 선택·외부 모델 컨트롤, provider/source 선택기 보존·개발자 route API와 정리 수명주기 확인 |
| Chromium UI 회귀 검사 | ✅ 완료 | 실제 `settings.html`, SillyTavern 1.18.0 고정 CSS, 기본 모델 등록·중복 방지·현재 선택 보존과 브라우저 native 재사용·공용 provider hook fixture를 확인하는 Playwright Chromium UI 회귀 검사 27개 통과 |
...
| v0.6.20 | ✅ 현재 릴리스·🧪 사용자 검증 대기 | 기본 목록 자동 제공, 등록·백업 분리, 중복 방지·동적 갱신·직접 등록 우선 |
