...
현재 단계: **별칭 없는 용도별 라우팅과 Connection Profile 어댑터 구현 완료, v0.5.0 안정화 진행 예정**
...
| 자동 검사 | ✅ 완료 | 단위·통합·수명주기·버전 일치 검사 68개 통과 |
| 사용자 체크리스트 | ✅ 완료 | 공통 항목, 24개 제공업체 조건부 매트릭스, Z.AI (GLM) 특별 항목 제공 |
| 공개 Registry API | ✅ 완료 | 전역 1.1.0 계약, 복합키 조회·변경, 불변 스냅샷, 이벤트와 종료 동작 구현 |
| 용도별 보조 라우팅 | ✅ 완료 | Registry 모델과 Connection Profile 직접 연결, 메인 모델 비변경, 무음 대체 금지 구현 |
...
- `globalThis.CustomModelRouter` 전역과 API 계약 `1.1.0`
...
- SillyTavern Connection Profile을 재사용하는 내장 어댑터와 다른 확장의 opt-in 어댑터 계약
- 어댑터가 없을 때 안전한 기본 동작
- 주 채팅 모델을 바꾸지 않는 보조 요청 라우팅

구현 결과:

- `translation`, `summary`, `search`, `captioning`, `custom`과 사용자 정의 용도
- `{ provider, modelId, adapterId, connectionProfileId }` 직접 경로
- 공개 Routing API 1.0.0과 어댑터 등록·해제·AbortSignal 계약
- Connection Profile provider 일치 검증과 `model` 단독 override
- 경로·모델·프로필·어댑터 오류 시 무음 대체 금지
- 패널 경로 저장·해제·사용자 실행 테스트

완료 조건: 지원 대상으로 명시한 어댑터 통합 검증을 통과하고 보조 요청 실패가 주 모델 설정을 바꾸지 않음을 확인합니다. — ✅ 자동 통합 검사 충족, 실제 계정 요청은 사용자 검증 대기
...
| v0.2.1 | ✅ 게시 | 불필요한 중간 기능 단계를 제거하고 이후 기능 버전을 재정렬 |
| v0.3.0 | ✅ 게시 준비 | 공개 Registry API 1.0.0, 불변 스냅샷과 변경 이벤트 제공 |
| v0.4.0 | ✅ 현재 | Connection Profile 어댑터와 별칭 없는 용도별 직접 모델 라우팅 제공 |
