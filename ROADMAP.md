...
현재 단계: **v0.6.10의 대량 모델 관리·삭제 실행 취소·백업 미리보기·복구 상세와 예외 중심 외부 관리·DOM option 경고를 구현한 뒤 실제 사용자 환경 검증 대기 중**
...
| 관리 팝업과 기본 컨트롤 | ✅ 완료 | 24개 제공업체 모델 등록·삭제, 전체 provider 목록, 12개 초과 조건부 검색, 최대 200줄 원자 등록, 삭제 실행 취소와 6개 초과 숨은 스크롤 |
...
| 자동 검사 | ✅ 완료 | 단위·통합·수명주기·보안 경계·버전 일치 검사 207개 통과 |
...
12. 관리 목록은 전체 provider 모델을 기본 표시하며 6개 초과 숨은 스크롤, 12개 초과 조건부 검색, 최대 200줄 원자 등록, 삭제 실행 취소, 작은 삭제 버튼과 공식 닫기 하나를 유지합니다.
...
16. GitHub Actions의 UI 회귀 검사 증거와 실제 설치 화면을 비교해 SillyTavern 런타임에서만 생기는 차이가 없는지 확인합니다.

실제 검증에서 발견되는 v0.6 범위의 후속 결함은 `v0.6.11`, `v0.6.12`, ... 패치 버전으로 수정합니다.
...
| v0.6.10 | ✅ 현재 릴리스·🧪 사용자 검증 대기 | 대량 모델 관리, 삭제 실행 취소, 백업 미리보기·복구 상세, 예외 중심 외부 관리와 DOM option 경고 |
