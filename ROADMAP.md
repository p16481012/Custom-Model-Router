...
현재 단계: **v0.6.18 중복 정리 바로가기·버튼 폭 수정 및 자동 검사 완료, 실제 설치 화면 검증 대기**
...
| 자동 검사 | ✅ 완료 | 단위·통합·수명주기·보안 경계·버전 일치 검사 256개 통과 |
| DOM·공개 API 샌드박스 | ✅ 완료 | 기본 24개·native 선택·외부 모델 컨트롤, provider/source 선택기 보존·개발자 route API와 정리 수명주기 확인 |
| Chromium UI 회귀 검사 | ✅ 완료 | 실제 `settings.html`, SillyTavern 1.18.0 고정 CSS, 공식 닫기·중복 정리 배치와 브라우저 native 재사용·공용 provider hook fixture를 확인하는 Playwright Chromium UI 회귀 검사 24개 통과 |
...
| v0.6.18 | ✅ 현재 릴리스·🧪 사용자 검증 대기 | 목록 제목 옆 중복 정리 아이콘, 독립 미리보기와 초점 복원, 텍스트 버튼 폭 수정 |
