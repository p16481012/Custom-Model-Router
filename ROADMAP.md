...
현재 단계: **v0.6.5에서 관리 UI를 정리하고 외부 직접 연결 회귀를 복구해 사용자 검증 대기 중**
...
| 관리 팝업과 기본 컨트롤 | ✅ 완료 | 24개 제공업체 모델 등록·삭제, 전체 provider 목록 기본 표시, 6개 초과 숨은 스크롤, 헤더 닫기와 문장 단위 줄바꿈 |
...
| 범용 외부 확장 브리지 | ✅ 완료 | 표준 select/input/datalist 탐지, provider별 자동 연결, 모든 provider를 표시하는 직접 연결, 연결 안 함, 재렌더·선택 복원 |
| 설정 백업·복구 | ✅ 완료 | Registry·route·외부 mode·선택을 portable schema v2로 처리, legacy provider mapping을 직접 연결로 이관, v1 이관·미래 schema 거부 |
| 안정성 계측 | ✅ 완료 | source·profile 전환 표본의 observer·listener·group 누적 판정 |
| 자동 검사 | ✅ 완료 | 단위·통합·수명주기·보안 경계·버전 일치 검사 146개 통과 |
| 샌드박스와 자동 통합 검사 | ✅ 완료 | 동작 샌드박스는 기본 24개·native 선택·외부 자동 추론·개발자 route API를 확인. 직접 연결의 provider별 모델은 자동 통합 검사로 확인했으며, 좁은 화면 배치는 실제 런타임 검증이 아닌 수동 DOM/CSS 샌드박스로 분리 |
...
7. 백업 round trip 뒤 Registry·route·외부 직접 연결·연결 안 함·provider별 선택이 복원되고 연결 비밀은 남지 않습니다.
8. 외부 확장 모델 컨트롤에서는 등록 ID가 표시될 뿐 아니라 실제 요청의 `model`에 전달됩니다.
9. 비대상 모델 컨트롤은 변경되지 않고, provider를 판별할 수 없는 표준 Chat Completion 대상은 직접 연결하기 전까지 변경되지 않습니다.
...
12. 관리 목록은 전체 provider 모델을 기본 표시하며 6개 초과 숨은 스크롤, 작은 삭제 버튼, 헤더 닫기와 문장 단위 줄바꿈을 유지합니다.
13. 외부 대상의 자동·직접 연결·연결 안 함이 재렌더와 새로고침 뒤에도 유지됩니다.

실제 검증에서 발견되는 v0.6 범위의 후속 결함은 `v0.6.6`, `v0.6.7`, ... 패치 버전으로 수정합니다.
...
| v0.6.5 | ✅ 현재 릴리스·🧪 사용자 검증 대기 | 전체 등록 목록·UI 정리, 외부 직접 연결과 연결 안 함 회귀 복구 |
