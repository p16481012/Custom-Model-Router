...
test('배포 파일과 진행 문서의 버전 표기가 모두 일치한다', async () => {
    ...
    assert.match(providerIntegrationSandbox, /src\/provider-integrations\.js\?provider-sandbox=0\.6\.22/);
    ...
    const currentUiCountPatterns = [
        ...
        /## v0\.6\.22[\s\S]*?Node 자동 검사 \d+개와 Playwright Chromium UI 회귀 검사 (\d+)개로/,
        /v0\.6\.22 저장소에서는 Node 자동 검사 \d+개와 Playwright Chromium UI 회귀 검사 (\d+)개를/,
    ];
    ...
    assert.equal(documentedUiCounts[0], 31, 'v0.6.22 Playwright UI 검사 개수는 31개여야 한다');
