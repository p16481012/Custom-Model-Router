# v0.5.0 통합 사용자 검증 체크리스트

대상 버전: **v0.5.0**

이 문서는 v0.1~v0.5 기능을 한 번에 확인하는 최종 수동 검증 순서입니다. 위에서 아래로 진행하고, 사용하지 않는 제공업체·개발자 API 항목은 `해당 없음`으로 표시하세요.
...
- [ ] **[필수][MIG-02] 관리 팝업에 `v0.5.0` 배지가 표시된다.**
...
- [ ] **[필수][BKP-01] `백업 내보내기`로 `custom-model-router-backup-v0.5.0.json`을 저장한다.**
- [ ] **[필수][BKP-02] JSON 최상위에 `format`, `schemaVersion`, `createdAt`, `registry`, `purposeRoutes`만 있는지 확인한다.**
...
- [ ] **[필수][BKP-07] 다시 원본 백업을 가져와 확인하면 Registry와 route가 내보내기 당시 상태로 복원된다.**
...
```text
제목: [v0.5.0][제공업체 또는 기능][항목 ID] 짧은 증상
...
실패 항목이 있어도 데이터 손실·무한 반복·비밀 노출이 아니라면 나머지 독립 항목은 계속 확인해도 됩니다. 결과를 모으면 같은 v0.5 범위의 수정은 `v0.5.1`, `v0.5.2`, ...로 반영합니다.
