...
- [ ] **[필수][MIG-02] 관리 팝업에 `v0.6.1` 배지가 표시된다.**
...
- [ ] **[권장][UI-10] 좁은 창 또는 모바일 폭에서 입력·버튼·기능별 모델 지정·진단 섹션을 가로 스크롤 없이 사용할 수 있다.**
- [ ] **[필수][UI-10A] 일반 팝업 폭에서 기능별 모델 지정의 `저장·지정 해제·연결 테스트` 버튼이 같은 행·같은 너비로 표시되고, 매우 좁은 폭에서는 겹치지 않고 `2+1`로 정렬된다.**
- [ ] **[필수][UI-10B] `상태 확인·결과 복사·백업 저장·백업 불러오기` 문구가 가로로 표시되고 한 글자씩 세로로 찌그러지지 않는다.**
- [ ] **[필수][UI-10C] 모델 등록과 세 추가 기능이 카드로 구분되고 카드 안쪽과 카드 사이에 충분한 여백이 있다.**
- [ ] **[필수][UI-10D] `다른 확장에서도 모델 사용 → 연결 상태 설명 보기`를 열면 자동으로 연결됨·직접 지정·설정 필요·연결 안 함의 의미가 설명되어 있다.**
- [ ] **[필수][UI-10E] 좁은 화면에서도 현재 모델 행의 `현재 사용` 텍스트가 남고 색상만으로 상태를 구분하지 않는다.**
- [ ] **[필수][UI-10F] Tab 이동에서 숨겨진 파일 입력에 초점이 멈추지 않고 `백업 불러오기` 버튼으로 파일 선택창을 열 수 있다.**
...
먼저 CMR Registry에 외부 확장에서 시험할 제공업체의 사용자 모델을 하나 등록합니다. 관리 팝업의 **다른 확장에서도 모델 사용**을 열고, 실제로 사용하는 외부 확장의 Chat Completion 모델 입력란을 확인합니다.

- [ ] **[조건부][EXT-01] 표준 Chat Completion `select`를 가진 외부 확장을 찾고, 제공업체가 확실하면 `사용 가능`과 `자동으로 연결됨`이 함께 표시된다.**
...
- [ ] **[필수][EXT-04] 제공업체를 확정하지 못한 모델 입력란은 `설정 필요`로 표시되고 CMR 옵션을 임의로 주입하지 않는다.**
- [ ] **[조건부][EXT-05] `설정 필요` 항목을 열고 `연결 방식`에서 제공업체를 직접 지정하면 해당 provider의 모델만 나타나고 자동 판단보다 직접 지정이 우선한다.**
- [ ] **[조건부][EXT-06] `연결 방식`을 직접 지정 → 연결 안 함 → 자동 감지 다시 시도 순서로 바꾸면 CMR 옵션이 제거됐다가 안전한 자동 판단 상태로 돌아오며 각 상태가 정확히 표시된다.**
...
- [ ] **[권장][EXT-16] 관리 팝업의 `사용 가능·설정 필요·연결 안 함` 개수와 안전을 위해 건드리지 않은 비채팅 입력란 개수가 실제 화면과 일치한다.**
- [ ] **[필수][EXT-17] 외부 확장 연결 전후 메인 Chat Completion source·모델·API 키·endpoint가 바뀌지 않는다.**
- [ ] **[선택][EXT-18] React 자체 위젯, iframe, 닫힌 Shadow DOM 또는 모델 control 없는 확장이 자동 지원 대상으로 잘못 표시되지 않으며 전용 opt-in이 필요하다는 안내를 확인한다. 용도를 드러내지 않는 일반적인 비채팅 모델 control이 잘못 감지되면 연결 안 함으로 지정한다.**
...
- [ ] **[필수][DIAG-01] 문제 확인 및 설정 백업 섹션에서 `상태 확인`을 누른다.**
...
- [ ] **[필수][DIAG-04A] 외부 브리지 observer·listener·target 수와 사용 가능·설정 필요·연결 안 함·안전을 위해 건너뜀 수가 표시되고 설명 없는 증가가 없다.**
- [ ] **[필수][DIAG-05] 경고가 있다면 코드와 사유를 결과 메모에 기록한다.**
- [ ] **[필수][DIAG-06] `결과 복사` 내용을 텍스트 편집기에서 열어 API 키·endpoint·project/account ID·Service Account가 없는지 확인한다.**
...
- [ ] **[필수][BKP-01] `백업 저장`으로 `custom-model-router-backup-v0.6.1.json`을 저장한다.**
...
```text
제목: [v0.6.1][제공업체 또는 기능][항목 ID] 짧은 증상
...
실패 항목이 있어도 데이터 손실·무한 반복·비밀 노출이 아니라면 나머지 독립 항목은 계속 확인해도 됩니다. 결과를 모으면 같은 v0.6 범위의 후속 수정은 `v0.6.2`, `v0.6.3`, ...로 반영합니다.
