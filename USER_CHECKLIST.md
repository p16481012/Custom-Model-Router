...
- [ ] **[필수][MIG-02] 관리 팝업에 `v0.6.0` 배지가 표시된다.**
...
- [ ] **[필수][BKP-01] `백업 내보내기`로 `custom-model-router-backup-v0.6.0.json`을 저장한다.**
...
```text
제목: [v0.6.0][제공업체 또는 기능][항목 ID] 짧은 증상
...
실패 항목이 있어도 데이터 손실·무한 반복·비밀 노출이 아니라면 나머지 독립 항목은 계속 확인해도 됩니다. 결과를 모으면 같은 v0.6 범위의 수정은 `v0.6.1`, `v0.6.2`, ...로 반영합니다.
